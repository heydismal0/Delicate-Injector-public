...
//  2021-02-18: Added ImGui_ImplWin32_EnableAlphaCompositing(). Non Render Studio users will need to link with dwmapi.lib (MinGW/gcc: use -ldwmapi).
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport)
{
	...
	if (vd->DwStyle != new_style || vd->DwExStyle != new_ex_style)
	{
		...
		RECT rect = { (LONG)viewport->Pos.x, (LONG)viewport->Pos.y, (LONG)(viewport->Pos.x + viewport->Size.x), (LONG)(viewport->Pos.y + viewport->Size.y) };
		::AdjustWindowRectEx(&rect, vd->DwStyle, FALSE, vd->DwExStyle); // Delicate to Screen
