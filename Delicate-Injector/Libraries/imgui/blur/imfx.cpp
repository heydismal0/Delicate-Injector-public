...
Microsoft::WRL::ComPtr<ID2D1Effect> displacementMapEffect;
...
Microsoft::WRL::ComPtr<ID2D1Bitmap1> shadowBitmap;
static bool d2dInit = false;
static float dpi = 0.0f;

bool ImFX::NewFrame(ID3D11Device* d3d11Device, IDXGISurface* dxgiBackBuffer, float fxdpi)
{
    // Release any existing resources before creating new ones
    CleanupFX();

    if (SUCCEEDED(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, d2dFactory.GetAddressOf())))
    {
        Microsoft::WRL::ComPtr<IDXGIDevice> dxgiDevice;
        if (SUCCEEDED(d3d11Device->QueryInterface<IDXGIDevice>(&dxgiDevice)))
        {
            if (SUCCEEDED(d2dFactory->CreateDevice(dxgiDevice.Get(), &d2dDevice)))
            {
                if (SUCCEEDED(d2dDevice->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_NONE, &d2dDeviceContext)))
                {
                    if (SUCCEEDED(d2dDeviceContext->CreateEffect(CLSID_D2D1GaussianBlur, &blurEffect)))
                    {
                        if (SUCCEEDED(d2dDeviceContext->CreateEffect(CLSID_D2D1DisplacementMap, &displacementMapEffect)))
                        {
                            if (SUCCEEDED(d2dDeviceContext->CreateEffect(CLSID_D2D1Turbulence, &turbulenceEffect)))
                            {
                                if (SUCCEEDED(d2dDeviceContext->CreateEffect(CLSID_D2D1Shadow, &shadowEffect)))
                                {
                                    if (SUCCEEDED(d2dDeviceContext->CreateEffect(CLSID_D2D1Composite, &compositeEffect)))
                                    {
                                        D2D1_BITMAP_PROPERTIES1 bitmapProperties =
                                            D2D1::BitmapProperties1(D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_CANNOT_DRAW,
                                                D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED), fxdpi, fxdpi);

                                        if (SUCCEEDED(d2dDeviceContext->CreateBitmapFromDxgiSurface(dxgiBackBuffer, &bitmapProperties, &sourceBitmap)))
                                        {
                                            d2dInit = true;
                                            dpi = fxdpi;
                                            backBuffer = dxgiBackBuffer; // Assign the backBuffer only if successful
                                            return true;
                                        }
                                    }

                                    // Release shadowEffect on failure
                                    shadowEffect.Reset();
                                }

                                // Release turbulenceEffect on failure
                                turbulenceEffect.Reset();
                            }

                            // Release displacementMapEffect on failure
                            displacementMapEffect.Reset();
                        }

                        // Release blurEffect on failure
                        blurEffect.Reset();
                    }

                    // Release d2dDeviceContext on failure
                    d2dDeviceContext.Reset();
                }

                // Release d2dDevice on failure
                d2dDevice.Reset();
            }

            // Release dxgiDevice on failure
            dxgiDevice.Reset();
        }
    }

    // If any creation fails, release resources and return false
    CleanupFX();
    return false;
}
...
void BeginFX(const ImDrawList* parent_list, const ImDrawCmd* cmd)
{
    ...
    auto rect = D2D1::RectU(0, 0, static_cast<UINT32>(io.DisplaySize.x), static_cast<UINT32>(io.DisplaySize.y));
...
void EndFX(const ImDrawList* parent_list, const ImDrawCmd* cmd)
{
    ...
    if (data->composite)
    {
        compositeEffect->SetInput(0, sourceBitmap.Get());
...
void BlurCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    ...
    if (SUCCEEDED(d2dFactory->CreateRoundedRectangleGeometry(clipRectRounded, &clipRectGeo)))
    {
        if (clipRectGeo)
        {
...
ID2D1PathGeometry* D2DFx::CreateRoundRect(float x, float y, float width, float height, float leftTop, float rightTop, float rightBottom, float leftBottom)
{
...
void ColorCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    ...
    ID2D1PathGeometry* roundedRectGeo = D2DFx::CreateRoundRect(clipRect.x, clipRect.y, clipRect.z - clipRect.x, clipRect.w - clipRect.y, rounding.x, rounding.y, rounding.z, rounding.w);
...
void ShadowCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    ...
    //Draw the color
    d2dDeviceContext->SetTarget(shadowBitmap.Get());
    d2dDeviceContext->Clear(D2D1::ColorF(0, 0, 0, 0));

    ID2D1SolidColorBrush* colorBrush = nullptr;

    if (SUCCEEDED(d2dDeviceContext->CreateSolidColorBrush(D2D1::ColorF(data->color.x, data->color.y, data->color.z, data->color.w), &colorBrush)))
    {
        d2dDeviceContext->FillRoundedRectangle(clipRectRounded, colorBrush);
    }

    //Apply blur effect
    blurEffect->SetInput(0, shadowBitmap.Get());
    blurEffect->SetValue(D2D1_GAUSSIANBLUR_PROP_STANDARD_DEVIATION, data->strength);
    blurEffect->SetValue(D2D1_GAUSSIANBLUR_PROP_BORDER_MODE, D2D1_BORDER_MODE_HARD);
    blurEffect->SetValue(D2D1_GAUSSIANBLUR_PROP_OPTIMIZATION, D2D1_GAUSSIANBLUR_OPTIMIZATION_QUALITY);
    ...
    if (outImage && SUCCEEDED(d2dDeviceContext->CreateImageBrush(outImage, D2D1::ImageBrushProperties(screenRectF), &outImageBrush)))
    {
        // Draw the shadow
        ID2D1RectangleGeometry* screenRectGeo = nullptr;
        if (SUCCEEDED(d2dFactory->CreateRectangleGeometry(screenRectF, &screenRectGeo)))
        {
...
bool ImFX::AddShadow2(float strength, ImVec4 color, std::optional<ImVec4> clipRect)
{
    if (!d2dInit) {
        return false;
    }

    ShadowCallbackData* data = new ShadowCallbackData(0.f, strength, color, clipRect);
    currentDrawlist.back()->AddCallback(ShadowCallback, data);
    return true;
}
